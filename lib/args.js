...
.option('--device-authorization', 'Perform OIDC Device Authorization Flow (`WATCHER_DEVICE_AUTHORIZATION=1`). Negate with `--no-device-authorization`.', getBoolean('WATCHER_DEVICE_AUTHORIZATION', false))
.option('--no-device-authorization', 'Do not perform OIDC Device Authorization Flow (`WATCHER_DEVICE_AUTHORIZATION=0`). This is the default behavior.')
